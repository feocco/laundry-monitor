...
5. Washer finish starts a transfer workflow. Dryer start clears it. If no dryer
   ...
   Jess a reminder with `Snooze 2h` and `Done` actions.
...
Lifecycle validation is a rollout aid. Joe receives yes/no prompts for selected
start and finish events. Responses are persisted for review and exposed through
`GET /v1/status`, but they do not alter detector state.
